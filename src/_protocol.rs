...
pub trait ProtocolDecoder {
    fn read_message_begin(&mut self) -> Result<(&str, ThriftMessageType), Error>;
    fn read_message_end(&mut self) {}
}

pub struct BinaryProtocolDecoder<R> {
    rd: R
}

impl<R: Read + ReadBytesExt> BinaryProtocolDecoder<R> {
    pub fn new(rd: R) -> BinaryProtocolDecoder<R> {
...
pub struct Message<'a> {
    name: &'a str,
    ty: ThriftMessageType,
    seqid: i32
}

impl<'a> Message<'a> {
    pub fn new(name: &'a str, ty: ThriftMessageType, seqid: i32) -> Message<'a> {
        Message {
            name: name,
            ty: ty,
            seqid: seqid
...
impl<R: Read + ReadBytesExt> ProtocolDecoder for BinaryProtocolDecoder<R> {
    fn read_message_begin(&mut self) -> Result<Message, Error> {
        let size = try!(self.rd.read_i32::<BigEndian>());

        if size < 0 {
            let version = size & THRIFT_VERSION_MASK as i16;
            if version != THRIFT_VERSION_1 {
                panic!("Thrift version does not match.");
            }
...
pub struct BinaryProtocolEncoder<'a> {
    wr: Serializer<'a>
}

impl<'a> BinaryProtocolEncoder<'a> {
    pub fn new(wr: &'a mut Write) -> BinaryProtocolEncoder<'a> {
...
impl<'a> ProtocolEncoder for BinaryProtocolEncoder<'a> {
    fn write_message_begin(&mut self, name: &str, message_type: ThriftMessageType) {
...
impl<'a> serde::Serializer for Serializer<'a> {
    ...
    fn serialize_bytes(&mut self, value: &[u8]) -> Result<(), Error> {
        self.serialize_i32(value.len() as i32);
        self.wr.write(value);
        Ok(())
    }
...
mod tests {
    ...
    use super::{ThriftMessageType, BinaryProtocolEncoder, ProtocolEncoder, ProtocolDecoder};
